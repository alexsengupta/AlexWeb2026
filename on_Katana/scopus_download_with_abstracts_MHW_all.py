...
# Broad query for all marine heatwave literature
# Note: bare "MHW" is excluded because it also matches "Mean High Water"
# in tidal/coastal research, producing many false positives.
SCOPUS_QUERY = (
    'TITLE-ABS-KEY('
    '"marine heatwave*" OR '
    '"marine heat wave*"'
...
OUTPUT_FILE = "scopus_all_MHW.csv"
SEARCH_CACHE = "scopus_all_MHW_search_cache.json"
...
def get_with_retries(url, *, params=None, headers=None, max_retries=5, base_delay=2.0):
    ...
    for attempt in range(max_retries):
        ...
        if status in (429, 500, 502, 503, 504):
            if attempt == max_retries - 1:
                print(f"Giving up after max retries (last status {status})")
                return None
            delay = base_delay * (2 ** attempt)
        ...
        print(f"Non-retryable error ({status}) from Scopus.")
        return None
...
def fetch_all_results():
    ...
    # Cache the search results so we don't need to re-fetch if the
    # abstract-fetching phase gets interrupted
    if results:
...
# -----------------------------
def main():
    print("Searching Scopus for all marine heatwave literature...")
